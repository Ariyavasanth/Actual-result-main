...
export class AdminScheduleTestComponent {
  ...
  schedule() {
    ...
    if (!this.model.userreview && this.model.reviewMode === 'scheduled') {
      const dateMatch = String(this.model.reviewDate || '').trim().match(/^(\d{2})\/(\d{2})\/(\d{4})$/);
      const timeMatch = String(this.model.reviewTime || '').trim().match(/^([01]\d|2[0-3]):([0-5]\d)$/);
      if (!dateMatch || !timeMatch) {
        notify('Review date and time are required in DD/MM/YYYY and HH:MM format', 'error');
        return;
      }
      const reviewAt = new Date(Number(dateMatch[3]), Number(dateMatch[2]) - 1, Number(dateMatch[1]), Number(timeMatch[1]), Number(timeMatch[2]));
      if (reviewAt.getFullYear() !== Number(dateMatch[3]) || reviewAt.getMonth() !== Number(dateMatch[2]) - 1 || reviewAt.getDate() !== Number(dateMatch[1])) {
        notify('Review date is invalid', 'error');
        return;
      }
